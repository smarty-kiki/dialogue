...
dialogue_topic([
    ...
    foreach ($column_names as $column_name) {
        ...
        // 枚举识别
        if (
            ends_with($column_name, '分类') ||
            ends_with($column_name, '类型') ||
            ends_with($column_name, '状态') ||
            ends_with($column_name, '阶段') ||
            ends_with($column_name, '种类')
        ) {
            $message = dialogue_ask_and_wait($user_info, '好的，'.$column_name.'是枚举吧？有哪些呢？一次发给我吧');
            if (! $message) return;
            $answer_str = $message['content'];

            if (! ends_with($answer_str, '不是')) {
        ...
        // 价格识别
        if (
            ends_with($column_name, '价格') ||
            ends_with($column_name, '价钱') ||
            ends_with($column_name, '金额') ||
            ends_with($column_name, '价') ||
            ends_with($column_name, '金') ||
            ends_with($column_name, '价值')
        ) {
        ...
        // 时间识别
        if (
            ends_with($column_name, '日') ||
            ends_with($column_name, '日期') ||
            ...
            if (
                ends_with($answer_str, '时间')
            ) {
                $entity_struct['datatype'] = 'datetime';
            }

            if (
                ends_with($answer_str, '日期')
            ) {
